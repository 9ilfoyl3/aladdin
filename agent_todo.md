...
- 存储上只多一个字段（几十字节），可以忽略
- **现在的 `ContextualEmbedder` 已经在做类似的事**，区别是把 header 生成放在 chunker 内部更准确（因为 chunker 知道标题层级），而当前是在 embedding 阶段从 metadata 拼接
...
**benchmark 验证的最优值：**
- child_size: **384-512**（当前 300 偏小，召回率受损）
- overlap: **80（~15% of chunk_size）**
- parent_size: **4096**（当前 1500 偏小，上下文不够）
...
**优化做法：**
- 当 LLM 一次返回多个 tool call 时，用并发池并发执行
- 结果按原始顺序收集
...
**优化做法：** 每轮 user message 前注入 XML 格式的 runtime context：
```xml
...
**优化做法：** CSV 转 Markdown 表格后走通用 chunker（按 chunk_size 切分），chunker 的 `headerTracker` 自动为后续 chunk prepend 表头。一个 chunk 包含多行数据。
...
- 触发时机：只要文件类型是 csv/xlsx/xls 就无条件触发
...
**优化做法：** Prompt 明确告诉 LLM 可以用正则 alternation：
...
## P1 — Agent 预设能力扩展（CustomAgent）

> 背景：「Agent 配置」预设（AgentPreset）已具备核心智能体配置能力。
...
**目标：** rag-qa / wiki-qa / hybrid-rag-wiki / data-analysis 等一键套用「系统提示词 + 工具 + KB 兼容性」组合。
...
**目标：** 智能体可选择启用的 MCP 服务（all / selected / none）。
...
**目标：** 渐进式披露的能力扩展，通过注入 system prompt 让 Agent 学会新能力。代码层有 `read_skill` 工具和 skill_manager。
