...
// React Bits Aurora（OGL 实现）：顶部流动的极光带。
// 颜色默认取系统强调绿调，blend=1（与卡片完全融合）。
// 参考 https://www.reactbits.dev/backgrounds/aurora
...
export default function Aurora({
  colorStops = ['#83d68e', '#3a7d44', '#a7e8b0'],
  amplitude = 1.0,
  blend = 1.0,
  speed = 0.6,
  className,
  ...
  useEffect(() => {
    ...
    // OGL 的 Triangle 默认带 uv 属性，Aurora 不需要，移除以免报错
    ...
    const update = (t: number) => {
      ...
      const { speed: s = 0.6, amplitude: amp = 1.0, blend: bl = 1.0 } = propsRef.current
      if (program) {
