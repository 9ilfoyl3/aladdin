...
export interface BentoCardData {
  ...
  label: string
  icon?: LucideIcon
  /** lg 布局下的额外 grid 跨度类名，用于拼出错落的 bento 布局 */
  spanClass?: string
}
...
const DEFAULT_GLOW_COLOR = '39, 255, 132'
...
const MagicBento: React.FC<MagicBentoProps> = ({
  ...
  return (
    <>
      ...
      <div
        className="bento-section relative grid select-none grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:auto-rows-[210px]"
        ref={gridRef}
      >
        {cards.map((card, index) => {
          const baseClassName = `card group relative flex h-full min-h-[200px] w-full flex-col justify-between overflow-hidden rounded-[20px] border border-white/10 bg-[#0a0f15]/70 p-6 transition-transform duration-300 ease-in-out hover:-translate-y-0.5 ${
          ...
          const inner = (
            <>
              ...
              <div className="relative mt-6">
                <h3 className={`mb-2 text-lg font-semibold text-white ${textAutoHide ? 'line-clamp-1' : ''}`}>
                  {card.title}
                </h3>
                <p className={`text-sm leading-relaxed text-white/55 ${textAutoHide ? 'line-clamp-2' : ''}`}>
                  {card.description}
                </p>
              </div>
            </>
